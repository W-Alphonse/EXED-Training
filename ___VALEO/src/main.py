...
if __name__ == "__main__" :
    ...
    X_train, X_test, y_train, y_test = train_test_split(X_df, y_df, test_size=0.2, random_state=48, stratify=y_df)  # shuffle=True,
                                        # A SUPPRIMER (recopier vers #3): fitted_model = pred.fit_predict_and_plot(X_train, y_train, X_test, y_test, [ValeoModeler.BRFC])
    ...
    # 2.d - Fit using RandomizedSearchCV
    fitted_model = pred.fit_cv_grid_or_random_search(X_train, y_train, [C.BRFC], n_random_iter=200, n_splits=8)
    fitted_model = pred.fit_cv_grid_or_random_search(X_train, y_train, [C.LRC], n_random_iter=200, n_splits=8)
    fitted_model = pred.fit_cv_grid_or_random_search(X_train, y_train, [C.GBC], n_random_iter=200, n_splits=8)

    # 3 - Predit and Plot using the TestSet
