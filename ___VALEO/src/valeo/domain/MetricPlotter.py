...
class MetricPlotter :
    ...
    def plot_roc(self, y_test, y_pred, clfTypes:[str]):
        ...
        plt.plot(roc[0], roc[1], color='darkorange', lw=lw, label='Courbe ROC_AUC (area = %0.4f)' % roc_auc_score(y_test, y_pred))
    ...
    def plot_precision_recall(self, y_test, y_pred, clfTypes:[str]):
        ...
        # plt.plot(pr[0], pr[1], color='darkorange', lw=lw, label='Coube Precision Recall (area = %0.4f)' % average_precision)
        plt.plot(pr[0], pr[1], color='darkorange', lw=lw, label='Coube Precision Recall')
        plt.xlim([0.0, 1.05])
        plt.ylim([0.0, 1.05])
        plt.xlabel('Recall')
